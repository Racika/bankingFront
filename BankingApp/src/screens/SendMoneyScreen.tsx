...
export default function SendMoneyScreen({ navigation }: any) {
  ...
  const load = async () => {
    ...
    const res = await axios.get(
      "https://racika-banking-api.onrender.com/api/me",
      {
  ...
  const loadRecent = async () => {
    try {
      ...
      const unique = Array.from(
        new Map(
          res.data.map((r: any) => [r.requested_number, r])
        ).values()
      );
  ...
  const doTransfer = async () => {
    ...
    try {
      const res = await fetch(
        "https://racika-banking-api.onrender.com/api/transferMoney",
        {
          method: "POST",
          headers: {
  ...
  const sendRequest = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem("token");
      await fetch(
        "https://racika-banking-api.onrender.com/api/requestPayment",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    try {
      const res = await fetch(
        "https://racika-banking-api.onrender.com/api/transferMoney",
        {
          ...
          body: JSON.stringify({
            fromCard: String(cardnum),
            toCard: String(req.requester_cardnum),
            amount,
          }),
        }
      );
      ...
      await deleteRequest(req.request_id, false);
      setResultMsg(`Sent $${amount} to ${req.requester_name}`);
      setResultColor(theme.primary);
      setResultVisible(true);
      await load();
    } catch {
  ...
  const deleteRequest = async (id: any, showResult = true) => {
    ...
    await fetch(
      "https://racika-banking-api.onrender.com/api/deleteRequest",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const insufficient =
    Number(amount) > (funds ?? 0) && mode === "send";
  ...
  return (
    ...
    >
      ...
      <Text
        style={[styles.balance, { color: theme.primary }]}
      >
        Balance: ${funds}
      </Text>
      ...
      >
        ...
        <TouchableOpacity
          style={[
            styles.toggleBtn,
            mode === "request" && { backgroundColor: theme.primary },
          ]}
          ...
          >
            {recent.map((r, i) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  setRecipientCard(r.requested_number)
                }
              >
      ...
      <TouchableOpacity
        style={[
          ...
          {
            backgroundColor:
              insufficient && mode === "send"
                ? "#666"
                : theme.primary,
          },
        ]}
        onPress={() =>
          validateInputs() && setConfirmVisible(true)
        }
      >
        <Text
...
const styles = StyleSheet.create({
  ...
  toggleBtn: { flex: 1, paddingVertical: 10, paddingHorizontal: 14, alignItems: "center", borderRadius: 12 },
  ...
  button: { paddingVertical: 10, paddingHorizontal: 15, borderRadius: 10, alignItems: "center", marginTop: 10 },
